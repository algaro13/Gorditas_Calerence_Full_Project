  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
        ...
        <h1 className="text-xl font-bold text-gray-900 mb-2">Confirma tu correo</h1>
        ...
        {aviso && <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-2 rounded-lg text-sm mb-4">{aviso}</div>}
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={yaVerifique}
            disabled={enviando}
            className="flex items-center justify-center gap-2 px-6 py-3 bg-orange-600 text-white rounded-lg font-medium hover:bg-orange-700 disabled:opacity-50"
          >
            <RefreshCw className={`w-4 h-4 ${enviando ? 'animate-spin' : ''}`} /> Ya confirmé mi correo
          </button>
          <button onClick={reenviar} disabled={enviando} className="px-6 py-2 text-orange-700 hover:text-orange-900 text-sm disabled:opacity-50">
            Volver a enviarme el enlace
          </button>
          <button onClick={() => void logout()} className="flex items-center justify-center gap-2 px-6 py-2 text-gray-500 hover:text-red-600 text-sm">
